...
## Документирование изменений (HOW-IT-WORK.md)

**ОБЯЗАТЕЛЬНО:** После выполнения каждого промпта пользователя, файл `HOW-IT-WORK.md` должен быть **полностью перезаписан** с описанием логики работы нового/изменённого кода с упором на объяснение концепций и причин такого подхода для джуна фронтендера

**Что писать:**
- Какие файлы были изменены/созданы
- Логика работы нового кода (что делает, как работает)
- Связи между компонентами (если затронуто несколько файлов)
...
## Изменённые файлы
- `path/to/file.js` — описание изменений

## Логика работы
[Подробное описание логики нового кода]
```

**Правила:**
- Файл перезаписывается **целиком** при каждом промпте (не дополняется)
- Описывай только изменения текущего промпта
- Файл в `.gitignore` — не попадает в репозиторий
