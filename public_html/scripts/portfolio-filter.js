document.addEventListener('DOMContentLoaded', function() {
  ...
  const typeFilter = document.querySelector('select[data-filter="type"]') || document.querySelector('select:first-of-type');
  const areaFilter = document.querySelector('input[type="number"]');
  const priceFilter = document.querySelector('select[data-filter="price"]') || document.querySelector('select:nth-of-type(2)');
  ...
  portfolioItems.forEach((item, index) => {
    if (index >= 2) { // Начинаем с 3-й карточки (индекс 2)
      item.style.opacity = '0';
      item.style.transform = 'translateY(20px)';
      item.style.transition = 'opacity 0.6s ease, transform 0.6s ease';
      observer.observe(item);
    } else { // Первые две карточки сразу видны
