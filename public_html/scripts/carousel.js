...
// Инициализация карусели отзывов
  ...
  const reviewsCards = reviewsTrack ? reviewsTrack.querySelectorAll('div[style*="position:relative"]') : [];
  const reviewsVisible = 4;
  function reviewsUpdate() {
    const card = reviewsCards[0];
    let cardWidth = card ? card.offsetWidth : 0;
    if (reviewsTrack) reviewsTrack.style.transform = `translateX(${-reviewsCurrent * (cardWidth + 24)}px)`;
  }
  if (reviewsCarousel && reviewsTrack && reviewsLeftBtn && reviewsRightBtn) {
    reviewsLeftBtn.addEventListener('click', function() {
      if (reviewsCurrent > 0) { reviewsCurrent--; reviewsUpdate(); }
    });
    reviewsRightBtn.addEventListener('click', function() {
      if (reviewsCurrent < reviewsCards.length - reviewsVisible) { reviewsCurrent++; reviewsUpdate(); }
    });
